// Modified from https://github.com/advename/web-timing-safe-equal
...
async function webTimingSafeEqual(left, right, options = {}) {
  ...
  if (leftUint8Array.length !== rightUint8Array.length || leftUint8Array.length === 0) {
  ...
  const [leftHMAC, rightHMAC] = await Promise.all([
    hmac(secretKey, leftUint8Array, options?.hmacAlgorithm ? options.hmacAlgorithm : undefined),
    hmac(secretKey, rightUint8Array, options?.hmacAlgorithm ? options.hmacAlgorithm : undefined),
  ])
...
async function hmac(secretKey, message, algorithm = 'SHA-256') {
  ...
  if (secretKey?.constructor?.name !== 'CryptoKey') {
    ...
    cryptoKey = await subtleCrypto.importKey('raw', keyUint8Array, { name: 'HMAC', hash: algorithm }, false, ['sign'])
  }
  // Sign the message with HMAC and the CryptoKey
  const signature = await subtleCrypto.sign('HMAC', cryptoKey, messageUint8Array)
 ...
 * @returns {CryptoKey}
 */
async function generateSecretKey({ keyLength = 64, algorithm = 'SHA-256' } = {}) {
  // Validate algorithm
  validateAlgorithm(algorithm)
...
function validateValueOrConvertToUint8Array(value) {
  if (!(typeof value === 'string' || value instanceof String || value instanceof Uint8Array)) {
    // Neither string nor Uint8Array
    throw new Error(`Invalid comparison value: ${value} of type ${typeof value} is not of type String or Uint8Array`)
  }
